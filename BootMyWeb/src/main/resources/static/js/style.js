...
function getCategory_List(e) {
	...
	// 카테고리 선택 시 prodCategory 값을 업데이트하는 함수
	$(document).on('click', '.categoryList a', function(event) {
		event.preventDefault();
		var categoryData = $(this).data('set');
		var categoryId = categoryData.categoryId;  // 여기서 'categoryId'로 수정
		var groupId = categoryData.groupId;        // 여기서 'groupId'로 수정
		$("input[name='prodCategory']").val(groupId + categoryId);
		console.log("카테고리 선택됨: " + groupId + categoryId);
	});
